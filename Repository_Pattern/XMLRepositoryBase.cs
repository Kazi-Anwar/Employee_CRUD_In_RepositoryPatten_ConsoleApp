        ...
        where TContext : XMLSet<TEntity> where TEntity : class
    {
        ...
        public bool Delete(TKey id)
        {
            try
            {
                ...
                items.Remove(items.First(f => f.EmployeeID.Equals(id)));
        ...
        public TEntity Get(TKey id)
        {
            ...
            {
                List<IEntity<TKey>> items = m_context.Data as List<IEntity<TKey>>;
                return items.FirstOrDefault(f => f.EmployeeID.Equals(id)) as TEntity;
            }
            catch (Exception)
            {
                return null;
        ...
        public bool Update(TEntity model)
        {
            try
            {
                ...
                List<IEntity<TKey>> items = m_context.Data as List<IEntity<TKey>>;
                items.Remove(items.FirstOrDefault(f => f.EmployeeID.Equals(imodel.EmployeeID)));
                items.Add(imodel);
                m_context.Data = items as ICollection<TEntity>;
                return true;
            }
